...
class GremlinTests(unittest.TestCase):
    ...
    def test_can_add_functon_two_args(self):
        ...
        string      = str(g)
        expected    = 'g.function(%s, %s)' % (get_dict_key(g.bound_params, 'one'),
                                              get_dict_key(g.bound_params, 'two'))
    ...
    def test_can_add_raw_functon_two_args(self):
        ...
        expected = 'gfunction(%s, %s)' % (get_dict_key(g.bound_params, 'arg'),
                                          get_dict_key(g.bound_params, 'two'))
 
        self.assertEquals(string, expected)
        self.assertEquals(len(g.bound_params), 2)

    def test_can_add_functon_three_args(self):
        g           = Gremlin().function('one', 'two', 'three')
        string      = str(g)
        expected    = 'g.function(%s, %s, %s)' % (get_dict_key(g.bound_params, 'one'),
                                                  get_dict_key(g.bound_params, 'two'),
                                                  get_dict_key(g.bound_params, 'three'))
 
        self.assertTrue(string == expected)
        self.assertTrue(len(g.bound_params) == 3)
...
class GremlinInjectionTests(unittest.TestCase):
    ...
    def test_can_nest_with_bound_params(self):
        ...
        string   = str(g)
        params   = g.bound_params
        expected = 'g.function(%s, %s).nest(setSubProp(%s, %s))' % (get_dict_key(params, 'name'),
                                                                    get_dict_key(params, 'parent'),
                                                                    get_dict_key(params, 'prop'),
                                                                    get_dict_key(params, 'child'))
